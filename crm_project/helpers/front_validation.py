...
def validate_name(field_name, value, dialog):
    ...
    if not re.match(r"^[a-zA-ZÀ-ÿ' -]+$", value):
        QMessageBox.warning(dialog, "Input Error", f"Invalid {field_name}. Only letters, hyphens, and spaces are allowed.")
        return False
    return True
...
def validate_employee_number(employee_number, dialog):
    if not employee_number.isdigit():
        QMessageBox.warning(dialog, "Input Error", "Employee number must be a valid number.")
        return False
    return True
...
def validate_amount(field_name, value, dialog):
    try:
        ...
        QMessageBox.warning(dialog, "Input Error", f"{field_name} must be a positive number.")
        return False
    return True
...
def validate_event_name(name, dialog):
    ...
    if not re.match(r"^[a-zA-ZÀ-ÿ0-9' -]+$", name):
        QMessageBox.warning(dialog, "Input Error", "Invalid event name format. Only letters, numbers, hyphens, and spaces are allowed.")
        return False
    return True
...
def validate_attendees(attendees, dialog):
    try:
        ...
        QMessageBox.warning(dialog, "Input Error", "Attendees must be a positive integer.")
        return False
    return True
...
def validate_location(location, dialog):
    ...
    if not re.match(r"^[a-zA-ZÀ-ÿ0-9' -]+$", location):
        QMessageBox.warning(dialog, "Input Error", "Invalid location format. Only letters, numbers, hyphens, and spaces are allowed.")
        return False
    return True
...
def validate_comment(comment, dialog, max_length=500):
    ...
    if len(comment) > max_length:
        QMessageBox.warning(dialog, "Input Error", f"Comment cannot exceed {max_length} characters.")
        return False
    return True